            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-4">
                <form method="GET" action="{{ route('services.index') }}" class="flex flex-wrap items-end gap-4">
                    <div>
                        ...
                        <x-text-input id="search" name="search" type="text" class="mt-1" :value="$search" placeholder="Search services…" />
                    </div>

                    <x-secondary-button type="submit">{{ __('Filter') }}</x-secondary-button>

                    @if ($search)
                        <a href="{{ route('services.index') }}" class="text-sm text-gray-500 hover:text-gray-700">{{ __('Clear') }}</a>
                    @endif
                </form>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($services as $service)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ number_format($service->unit_price, 2) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $service->tax_rate !== null ? number_format($service->tax_rate, 2).'%' : '—' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm space-x-3">
                                    @can('update', $service)
                                        <a href="{{ route('services.edit', $service) }}" class="text-indigo-600 hover:text-indigo-900">{{ __('Edit') }}</a>
                                    @endcan
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-4 text-sm text-gray-500 text-center">{{ __('No services found.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
